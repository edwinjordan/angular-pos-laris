...
export class AuthService {
 ...
 // endpoint: string = 'http://localhost:4000/api';
  ...
  signIn(user) {
    ...
    //   url,
    //   this.serialize({ "User_username" :user.User_username, "User_password" : user.User_password }, null),
    //   { headers: headers, responseType: "text" })
    // .toPromise().then((data : any) => { localStorage.setItem('user', JSON.stringify(data));  return data; });
    return new Promise((resolve, reject) => {
      this.http.post(url , { "User_username" :user.User_username, "User_password" : user.User_password } , { headers: { 'Content-Type': 'application/json' } })
        .subscribe(res => {
          resolve(res);
          // localStorage.setItem('user', JSON.stringify(data));  return data;
        }, (err) => {
          reject(err);
        });
    });
